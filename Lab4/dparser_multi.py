...
def sen_subj_verb_obj_cnt(sentence, subj_verb_obj_cnt):
    subjects    = [s for s in sentence if s['deprel'] == 'nsubj']
    objects     = [o for o in sentence if o['deprel'] == 'obj']
    for s in subjects:
        for o in objects:
            if s['head'] == o['head']:
                subj = s['form']
...
def perform_for(file):
    ...
    for sentence in formatted:
        ...
        for w in sentence:
            w['form'] = w['form'].lower()
            sentence_dict[w['id']] = w
        new_sentences += [sentence_dict]
...
if __name__ == "__main__":
    perform_for('ud-treebanks-v2.0/UD_Swedish/sv-ud-train.conllu')
    perform_for('ud-treebanks-v2.0/UD_Spanish/es-ud-train.conllu')
    perform_for('ud-treebanks-v2.0/UD_English/en-ud-train.conllu')
